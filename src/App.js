...
function Avatar({ email, size = 32 }) {
  return (
    <div style={{ width: size, height: size, borderRadius: t.radiusFull, background: 'linear-gradient(135deg, #1e3a5f 0%, #2563eb 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: size * 0.38, fontWeight: '700', fontFamily: t.font, flexShrink: 0, letterSpacing: '0.02em' }}>
      {getInitials(email)}
    </div>
  )
}
...
function ProfileDropdown({ user, onLogout }) {
  ...
  const viewTitle = { profile: 'My Profile', notifications: 'Notification Settings', support: 'Help & Support' }
          ...
          {view === 'menu' && (
            <>
              ...
              <div style={{ padding: '6px' }}>
                {[{ id: 'profile', icon: '👤', label: 'My Profile' }, { id: 'notifications', icon: '🔔', label: 'Notification Settings' }, { id: 'support', icon: '💬', label: 'Help & Support' }].map(item => (
                  <MenuItem key={item.id} onClick={() => setView(item.id)}>
                    <span style={{ fontSize: '14px', width: '18px' }}>{item.icon}</span>{item.label}
                  </MenuItem>
                ))}
              </div>
              <div style={{ padding: '6px', borderTop: `1px solid ${t.border}` }}>
                <MenuItem danger onClick={() => { setOpen(false); onLogout() }}>
                  <IconSignOut color={t.danger} size={14} />Sign Out
                </MenuItem>
              </div>
            </>
          )}
  ...
  return (
    <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: t.radiusLg, padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '6px', flex: 1, minWidth: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
        <span style={{ color: deltaColor, fontSize: '12px', fontWeight: '600' }}>{isPositive ? '↑' : '—'} {Math.abs(delta)}</span>
  ...
  if (isMobile) {
    return (
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: t.surface, borderTop: `1px solid ${t.border}`, display: 'flex', zIndex: 100 }}>
        ...
        <button onClick={onLogout} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 0', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: t.font, color: t.textMuted, borderTop: '2px solid transparent' }}>
        ...
        {navItems.map(({ id, label, Icon }) => {
          ...
          return (
            <button key={id} onClick={() => setActivePage(id)}
              style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 10px', borderRadius: t.radiusMd, cursor: 'pointer', border: 'none', background: active ? t.accentSubtle : 'transparent', color: active ? t.accent : t.textMuted, fontSize: '13px', fontWeight: active ? '600' : '500', fontFamily: t.font, width: '100%', textAlign: 'left', transition: 'background 0.1s, color 0.1s', justifyContent: hovered ? 'flex-start' : 'center', borderLeft: active ? `2px solid ${t.accent}` : '2px solid transparent' }}>
...
function LeadCard({ lead, brokerEmail, action, onActionChange }) {
  ...
  return (
    <div style={{ background: t.surface, borderRadius: t.radiusXl, overflow: 'hidden', border: `1px solid ${t.border}`, display: 'flex', flexDirection: 'column', borderTop: `2px solid ${STATUS_COLORS[status] || t.accent}` }}>
      ...
      <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginTop: '4px' }}>
          {['reached_out', 'follow_up', 'connected', 'not_interested'].map(s => (
            <button key={s} onClick={() => { handleStatus(s); if (s === 'follow_up') setShowFollowUp(true) }}
              style={{ border: `1px solid ${STATUS_COLORS[s]}`, borderRadius: t.radiusMd, padding: '4px 8px', fontSize: '10px', cursor: 'pointer', fontWeight: '600', fontFamily: t.font, background: status === s ? STATUS_COLORS[s] : 'transparent', color: status === s ? '#fff' : STATUS_COLORS[s], transition: 'all 0.1s' }}>
              {STATUS_LABELS[s]}
            </button>
          ))}
        </div>
        ...
        {followUpDate && status === 'follow_up' && <p style={{ color: t.warning, fontSize: '12px', margin: 0 }}>📅 Follow up: {new Date(followUpDate).toLocaleDateString()}</p>}
      </div>
    </div>
  )
}
...
function LeadsPage({ leads, actions, brokerEmail, onActionChange, loading, isMobile }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', padding: isMobile ? '12px 16px' : '14px 28px', borderBottom: `1px solid ${t.border}`, gap: '12px', flexWrap: 'wrap', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'flex-start' : 'center' }}>
        <div style={{ display: 'flex', gap: '6px' }}>
          {[{ id: 'new', label: 'New Leads' }, { id: 'not_interested', label: 'Not Interested' }].map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              style={{ background: activeTab === tab.id ? t.accent : 'transparent', color: activeTab === tab.id ? '#fff' : t.textMuted, border: `1px solid ${activeTab === tab.id ? t.accent : t.border}`, borderRadius: t.radiusMd, padding: isMobile ? '6px 12px' : '7px 14px', cursor: 'pointer', fontSize: '12px', fontWeight: '600', fontFamily: t.font }}>
              {tab.label}
            </button>
          ))}
        </div>
...
function AnalyticsPage({ leads, actions, isMobile }) {
  ...
  const card = { background: t.surface, borderRadius: t.radiusXl, padding: '20px', border: `1px solid ${t.border}` }
  const tooltip = { contentStyle: { background: t.elevated, border: `1px solid ${t.border}`, borderRadius: t.radiusMd, color: t.textPrimary, fontSize: '12px' } }
        ...
        <div style={{ background: t.bg, borderBottom: `1px solid ${t.border}`, padding: isMobile ? '10px 16px' : '12px 28px', display: 'flex', gap: isMobile ? '12px' : '0', overflowX: 'auto', flexShrink: 0, position: 'sticky', top: isMobile ? '49px' : '56px', zIndex: 40 }}>
          {statItems.map((s, i) => (
            <React.Fragment key={s.label}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', flexShrink: 0, padding: isMobile ? '0' : '0 28px 0 0' }}>
              ...
              {!isMobile && i < statItems.length - 1 && <div style={{ width: '1px', background: t.border, margin: '0 28px 0 0', alignSelf: 'stretch' }} />}
            </React.Fragment>
          ))}
        </div>
